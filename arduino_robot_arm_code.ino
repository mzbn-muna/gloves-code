...
void wakeUp() {
  if (loop_check == 0) {
    ...
    // Example: Start in parking position (already set in variables)
    Serial.println("Waking up...");

    // Smooth shoulder lift
    for (int pos = servo_joint_1_parking_pos_i; pos <= servo_joint_1_parking_pos_i + 80; pos += 2) {
      HCPCA9685.Servo(2, pos); // Shoulder
      HCPCA9685.Servo(0, servo_joint_L_parking_pos_i + (pos / 20)); // Elbow L follows slightly
      HCPCA9685.Servo(1, servo_joint_R_parking_pos_i - (pos / 20)); // Elbow R mirrors
      delay(20);
    }
    ...
    // Bend elbow smoothly
    for (int pos = servo_joint_L_parking_pos_i; pos <= servo_joint_L_parking_pos_i + 40; pos += 2) {
      HCPCA9685.Servo(0, pos);
      HCPCA9685.Servo(1, servo_joint_L_max_pos - pos); // mirror elbow
      delay(20);
    }

    delay(400);

    // Twist wrist CW and back
    for (int pos = servo_joint_3_parking_pos_i; pos >= servo_joint_3_parking_pos_i - 100; pos -= 5) {
      HCPCA9685.Servo(4, pos);
      delay(15);
    }
    for (int pos = servo_joint_3_parking_pos_i - 100; pos <= servo_joint_3_parking_pos_i; pos += 5) {
      HCPCA9685.Servo(4, pos);
      delay(15);
    }

    // Open and close gripper gently
    for (int pos = servo_joint_4_parking_pos_i; pos <= servo_joint_4_parking_pos_i + 40; pos += 5) {
      HCPCA9685.Servo(5, pos);
      delay(20);
    }
    for (int pos = servo_joint_4_parking_pos_i + 40; pos >= servo_joint_4_parking_pos_i; pos -= 5) {
      HCPCA9685.Servo(5, pos);
      delay(20);
    }
